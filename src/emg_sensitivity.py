...
# ---------------------------
def compute_progressive_scale(
    training: bool,
    current_epoch: int,
    base_scale: float,
    use_progressive_schedule: bool = True,
    schedule_start_epoch: int = 2,
    schedule_ramp_epochs: int = 4,
):
    """
    训练时：
      - 若关闭调度，则始终使用 base_scale
      - 若开启调度，则在 schedule_start_epoch 之前不启用术语先验，
        从 schedule_start_epoch 开始线性爬坡到 base_scale
    评估时：始终使用 base_scale
    返回: (use_term_attention, current_scale)
    """
    if not training:
        return True, base_scale

    if not use_progressive_schedule:
        return True, base_scale

    if current_epoch < schedule_start_epoch:
        return False, 0.0

    ramp_epochs = max(1, int(schedule_ramp_epochs))
    progress_ratio = min(1.0, (current_epoch - schedule_start_epoch + 1) / ramp_epochs)
    current_scale = base_scale * progress_ratio
...
# [修改] 数据处理：增加关键词乱序 + 子采样
# ==========================================
def make_collate_fn(tokenizer: BertTokenizer, max_length: int = 512, p_del: float = 0.5, shuffle_keywords: bool = True):
    try:
        import jieba
        have_jieba = True
    except Exception:
        ...
        for idx, it in enumerate(batch):
            ...
            if kw_list:
                ...
                if current_label == 1 and len(aug_kw_list) > 1:
                    if random.random() < p_del:  # 根据命令行参数控制关键词丢弃概率
...
class TermAttentionEnhancedBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None, term_mask=None, labels=None, class_weights=None):
        ...
        schedule_on, current_scale = compute_progressive_scale(
            training=self.training,
            current_epoch=self.current_epoch,
            base_scale=self.term_bias_scale,
            use_progressive_schedule=self.use_progressive_schedule,
            schedule_start_epoch=self.schedule_start_epoch,
            schedule_ramp_epochs=self.schedule_ramp_epochs,
        )
        use_term_attention = term_mask is not None and schedule_on
        ...
        if use_term_attention:
            ...
            term_bias = torch.where(term_mask.bool(), scaled_term_bias, -self.non_term_penalty * torch.ones_like(scaled_term_bias))
...
class TermSentBERT(nn.Module):
    ...
    def _compute_term_features(self, sequence_output, attention_mask, term_mask):
        ...
        if term_mask is None or not schedule_on:
            batch_size = sequence_output.size(0)
            device = sequence_output.device
            zero_features = torch.zeros(batch_size, self.term_feature_projection.out_features, device=device)
            zero_reg = torch.tensor(0.0, device=device, dtype=sequence_output.dtype)
            return zero_features, zero_reg
...
class TermSentParaBERT_V2_Fast(nn.Module):
    def __init__(self, pretrained_name='bert-base-chinese', dropout_prob=0.1, term_bias_scale: float = 2.0, non_term_penalty: float = 5.0,
                 use_progressive_schedule: bool = True, schedule_start_epoch: int = 2, schedule_ramp_epochs: int = 4):
        super().__init__()
        # 1. 加载 BERT
        self.bert = BertModel.from_pretrained(pretrained_name)
...
def measure_inference_latency_emg(model, dataloader, device, tokenizer=None, warmup_steps=10, measure_steps=50):
    model.eval()
    cached_batches = []
...
def main():
    ...
    parser.add_argument('--p_del', type=float, default=0.5, help='正样本关键词随机丢弃概率 Pdel')
    parser.add_argument('--non_term_penalty', type=float, default=5.0, help='非术语抑制常数 δ，对非术语 token 的注意力惩罚强度')
    parser.add_argument('--term_bias_scale', type=float, default=2.0, help='术语先验强度缩放系数')
    parser.add_argument('--disable_keyword_shuffle', action='store_true', help='关闭关键词乱序增强，仅用于敏感性/对照实验')
    parser.add_argument('--disable_progressive_schedule', action='store_true', help='关闭渐进式先验调度（仅用于有/无调度对照）')
    parser.add_argument('--schedule_start_epoch', type=int, default=2, help='渐进式先验开始生效的 epoch')
    parser.add_argument('--schedule_ramp_epochs', type=int, default=4, help='渐进式先验从开始到达到最大强度的爬坡轮数')
    ...
    logger.info(f"敏感性设置: p_del={args.p_del}, non_term_penalty={args.non_term_penalty}, term_bias_scale={args.term_bias_scale}, keyword_shuffle={not args.disable_keyword_shuffle}, progressive_schedule={not args.disable_progressive_schedule}, schedule_start_epoch={args.schedule_start_epoch}, schedule_ramp_epochs={args.schedule_ramp_epochs}")
    ...
    collate_fn = make_collate_fn(tokenizer, max_length=args.max_len, p_del=args.p_del, shuffle_keywords=(not args.disable_keyword_shuffle))
    # 修改 collate 函数返回原文
    ...
    if args.model_version == "v1":
        model = TermSentBERT(pretrained_name=args.model_name, term_bias_scale=args.term_bias_scale, non_term_penalty=args.non_term_penalty).to(device)
    elif args.model_version == "v2":
        model = TermSentParaBERT_V2_Fast(pretrained_name=args.model_name, dropout_prob=0.1, term_bias_scale=args.term_bias_scale, non_term_penalty=args.non_term_penalty).to(device)
    else:
        model = TermAttentionEnhancedBERT(pretrained_name=args.model_name, term_bias_scale=args.term_bias_scale, non_term_penalty=args.non_term_penalty).to(device)
    ...
    infer_ms, infer_sps = measure_inference_latency_emg(
        model=model,
    ...
    efficiency_metrics = {
        ...
        "p_del": args.p_del,
        "non_term_penalty": args.non_term_penalty,
        "term_bias_scale": args.term_bias_scale,
        "keyword_shuffle": (not args.disable_keyword_shuffle),
        "progressive_schedule": (not args.disable_progressive_schedule),
        "schedule_start_epoch": args.schedule_start_epoch,
