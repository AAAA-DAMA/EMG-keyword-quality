...
class TermSentParaGMUBERT(TermSentParaFusionBERT):
    """
    V4 优化版 (TermSentParaGMUBERT_Optimized)：
    1. 使用冻结的辅助 BERT 提取稳定的句/段特征（解决特征漂移）。
    2. 引入降维投影 (Bottleneck)，减轻 Cross-Attention 的参数负担（抗过拟合）。
    """
    def __init__(self, pretrained_name='bert-base-chinese', dropout_prob=0.3, # <--- 增大默认 Dropout 到 0.3
                 term_bias_scale=2.0, term_bias_reg_weight=5e-6,
                 internal_bias_scale=0.4, internal_bias_reg_weight=5e-6):
        # 初始化父类
        super(TermSentParaGMUBERT, self).__init__(pretrained_name, dropout_prob,
                                                  term_bias_scale, term_bias_reg_weight,
                                                  internal_bias_scale, internal_bias_reg_weight)
        
        hidden = self.bert.config.hidden_size
        
        # === 改进1：加载并冻结辅助 Frozen BERT (解决 Moving Target) ===
        logger.info("正在加载辅助 Frozen BERT 用于特征提取...")
        self.frozen_encoder = BertModel.from_pretrained(pretrained_name)
        for param in self.frozen_encoder.parameters():
            param.requires_grad = False
        self.frozen_encoder.eval()

        # === 改进2：降维 Cross-Attention (抗过拟合) ===
        # 将 768 维降到 256 维进行计算
        self.attn_dim = 256  
        ...
        # GMU 门控 (保持 3-way)
        self.gmu_gate = nn.Linear(hidden * 3, 3)

        # 分类器
        self.classifier = nn.Linear(hidden, 2)
        logger.info(f"[V4 Optimized] Initialized with Frozen Encoder & Bottleneck Attn (dim={self.attn_dim})")

    def _get_stable_features(self, raw_texts, tokenizer, device, max_len=128):
        """使用 frozen_encoder 提取特征的新辅助函数"""
        import re
        sent_batch_vecs = []
        doc_batch_vecs = []
        ...
        for text in raw_texts:
            # --- Document Level ---
            # 简单截断防止OOM
            enc_doc = tokenizer(text, return_tensors='pt', truncation=True, max_length=512).to(device)
            with torch.no_grad():
                out_doc = self.frozen_encoder(**enc_doc, return_dict=True)
            doc_vec = out_doc.pooler_output.squeeze(0) # (768)
            doc_batch_vecs.append(doc_vec)

            # --- Sentence Level ---
            sents = re.split(r'[。；;？！?!]', text)
            sents = [s.strip() for s in sents if s.strip()]
            if not sents: sents = [text[:max_len]]
            sents = sents[:5] # 限制最大句数
            
            curr_sents_vecs = []
            for s in sents:
                enc_sent = tokenizer(s, return_tensors='pt', truncation=True, max_length=max_len).to(device)
                with torch.no_grad():
                    out_sent = self.frozen_encoder(**enc_sent, return_dict=True)
                curr_sents_vecs.append(out_sent.pooler_output.squeeze(0))
            
            if curr_sents_vecs:
                sent_mean = torch.stack(curr_sents_vecs).mean(dim=0)
            else:
                sent_mean = torch.zeros_like(doc_vec)
            sent_batch_vecs.append(sent_mean)

        return torch.stack(sent_batch_vecs), torch.stack(doc_batch_vecs)

    def forward(self, input_ids, attention_mask, token_type_ids=None, term_mask=None,
                labels=None, class_weights=None, raw_texts=None, tokenizer=None):
        device = input_ids.device

        # 1. Main BERT (当前正在训练的主体)
        ...
        # 2. Extract Stable Features (使用冻结的 BERT)
        if raw_texts is not None and tokenizer is not None:
            sent_vec_raw, para_vec_raw = self._get_stable_features(raw_texts, tokenizer, device)
        else:
            sent_vec_raw = torch.zeros_like(pooled)
            para_vec_raw = torch.zeros_like(pooled)
            
        # 3. Projection & Cross Attention
        # Q 来自动态的 pooled, K/V 来自稳定的 sent/para
        
        q_token = self.proj_token_q(pooled).unsqueeze(1)       # (B, 1, 256)
        kv_sent = self.proj_sent_kv(sent_vec_raw).unsqueeze(1) # (B, 1, 256)
        kv_para = self.proj_para_kv(para_vec_raw).unsqueeze(1) # (B, 1, 256)

        sent_attn_out, _ = self.cross_sent(q_token, kv_sent, kv_sent) # (B, 1, 256)
        para_attn_out, _ = self.cross_para(q_token, kv_para, kv_para) # (B, 1, 256)

        # 还原维度 (256 -> 768)
        sent_feat = self.proj_sent_out(sent_attn_out.squeeze(1)) # (B, 768)
        para_feat = self.proj_para_out(para_attn_out.squeeze(1)) # (B, 768)

        # 4. GMU Fusion
        gmu_input = torch.cat([pooled, sent_feat, para_feat], dim=-1) # (B, 768*3)
        gates = torch.softmax(self.gmu_gate(gmu_input), dim=-1)
        self.last_gates = gates.detach().cpu()

        fused = (gates[:, 0:1] * pooled + 
                 gates[:, 1:2] * sent_feat + 
                 gates[:, 2:3] * para_feat)
...
def main():
    ...
    if args.model_version == "v1":
        ...
        model = TermSentParaGMUBERT(pretrained_name=args.model_name).to(device)
    else:
        model = TermAttentionEnhancedBERT(pretrained_name=args.model_name).to(device)

    # ===== 优化器 (修正版：修复 p.name 报错) =====
    no_decay = ["bias", "LayerNorm.weight"]
    
    # 1. 识别模型参数 ID
    bert_param_ids = list(map(id, model.bert.parameters()))
    frozen_param_ids = list(map(id, model.frozen_encoder.parameters())) if hasattr(model, 'frozen_encoder') else []
    base_param_ids = bert_param_ids + frozen_param_ids

    # 2. 分组 - 关键修改：同时保存 (name, param) 元组，而不是只保存 param
    # Group A: BERT 主体 (name, param)
    bert_tuples = [(n, p) for n, p in model.named_parameters() if id(p) in bert_param_ids and p.requires_grad]
    
    # Group B: 新增层 (name, param)
    new_tuples = [(n, p) for n, p in model.named_parameters() if id(p) not in base_param_ids and p.requires_grad]

    optimizer_grouped_parameters = [
        # Group A: BERT Body (小学习率 2e-5)
        # 注意：这里判断的是 n (name)，添加的是 p (param)
        {'params': [p for n, p in bert_tuples if not any(nd in n for nd in no_decay)], 
         'lr': args.lr, 'weight_decay': 0.01},
        {'params': [p for n, p in bert_tuples if any(nd in n for nd in no_decay)], 
         'lr': args.lr, 'weight_decay': 0.0},
        
        # Group B: New Layers (大学习率 2e-4)
        {'params': [p for n, p in new_tuples if not any(nd in n for nd in no_decay)], 
         'lr': args.lr * 10, 'weight_decay': 0.01},
        {'params': [p for n, p in new_tuples if any(nd in n for nd in no_decay)], 
         'lr': args.lr * 10, 'weight_decay': 0.0}
    ]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if dev_f1 > best_dev_f1:
            best_dev_f1 = dev_f1
            torch.save(model.state_dict(), os.path.join(args.output_dir, "best_model_112233.pt"))
            logger.info("更新最佳模型")

    # ===== 测试集 =====
    model.load_state_dict(torch.load(os.path.join(args.output_dir, "best_model_112233.pt")))
    ...
    save_predictions_csv(os.path.join(args.output_dir, "predictions_112233.csv"), ids, labels, preds, probs)
    save_gates_csv(os.path.join(args.output_dir, "gmu_gates_112233.csv"), gates)
