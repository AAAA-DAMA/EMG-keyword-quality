...
说明：
- xinjizuo.py / finallv2.py 内部会从 original_train 再切出内部 dev；
- original_dev 会被它们当作最终 test 使用；
- 因此这里不再支持 --train_path / --dev_path / --test_path 三文件模式。
"""
...
def extract_metrics(log_text: str):
    best_dev = _match_last([
        ...
        r"训练完成。最佳验证 F1=(\d+\.\d+)，出现于 epoch (\d+)",
        r"训练完成。最佳验证 F1=(\d+\.\d+),\s*出现于 epoch (\d+)",
    ], log_text, cast=str)
...
def main():
    ...
    print("=" * 80)
    print("开始运行外部领域 4 模型实验（clean 数据集双文件直传版）")
    ...
    for cfg in MODEL_CONFIGS:
        ...
        for seed in args.seeds:
            ...
            cmd = build_command(cfg=cfg, args=args, run_output_dir=run_output_dir, seed=seed)
