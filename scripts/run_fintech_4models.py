...
"""
兼容版四模型总控脚本（适配 xinjizuo.py / finallv2.py 只接受
--original_train / --original_dev 的情况）
...
- 如果你提供：
    ...
    --dev_path
    --test_path
  则自动把 train.json + dev.json 合并成一个临时 original_train，
  再把 test.json 作为 original_dev 传给原始训练脚本。
  这样：
    1) 保留外部 test
    2) 不需要改 xinjizuo.py / finallv2.py 的参数接口
    3) 训练脚本会自行从 merged train 里再切出内部 dev

注意：
- 这意味着“你预先构造的 dev.json”不会被直接作为训练脚本的 dev，
  而是并回 merged train 中，再由脚本内部切分。
- 如果你坚持“必须使用你自己那份 dev.json 作为显式验证集”，
  那就需要进一步修改 xinjizuo.py / finallv2.py 的数据加载逻辑。

输出：
- all_run_results.csv
- summary_by_model.csv
- paper_table_test_f1.csv
- paper_table_test_acc.csv
- summary.json
"""
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            try:
                ...
                raise ValueError(f"读取 JSONL 失败: {path}, line={line_no}, error={e}")
    return items
...
def prepare_compatible_paths(args, output_dir: Path):
    """
    返回：
    compatible_original_train, compatible_original_dev, prepare_meta
    """
    if args.original_train and args.original_dev:
        return Path(args.original_train), Path(args.original_dev), {
            "mode": "direct_original_paths"
        }

    if args.train_path and args.dev_path and args.test_path:
        train_items = read_jsonl(Path(args.train_path))
        dev_items = read_jsonl(Path(args.dev_path))
        merged = list(train_items) + list(dev_items)

        temp_dir = output_dir / "_temp_data"
        temp_dir.mkdir(parents=True, exist_ok=True)

        merged_train_path = temp_dir / "merged_original_train.json"
        compatible_test_path = temp_dir / "compatible_original_dev_test.json"

        write_jsonl(merged_train_path, merged)
        # 这里不拷贝，用统一名字保存一份，方便日志和复现实验
        test_items = read_jsonl(Path(args.test_path))
        write_jsonl(compatible_test_path, test_items)

        return merged_train_path, compatible_test_path, {
            "mode": "merge_train_dev_then_use_test_as_original_dev",
            "input_train_path": args.train_path,
            "input_dev_path": args.dev_path,
            "input_test_path": args.test_path,
            "merged_train_items": len(merged),
            "test_items": len(test_items),
        }

    raise ValueError(
        "你需要提供两种参数之一：\n"
        "1) --original_train + --original_dev\n"
        "或\n"
        "2) --train_path + --dev_path + --test_path"
    )
...
def build_command(
    ...
    run_output_dir: Path,
    seed: int,
    compatible_original_train: Path,
    compatible_original_dev: Path,
):
    if cfg["script_type"] == "baseline":
        script_path = args.baseline_script
    else:
        script_path = args.emg_script

    cmd = [
        "python", script_path,
        "--original_train", str(compatible_original_train),
        "--original_dev", str(compatible_original_dev),
...
def main():
    ...
    parser.add_argument("--original_dev", type=str, default=None)
    ...
    compatible_original_train, compatible_original_dev, prepare_meta = prepare_compatible_paths(args, output_dir)

    with open(output_dir / "data_prepare_meta.json", "w", encoding="utf-8") as f:
        json.dump({
            "compatible_original_train": str(compatible_original_train),
            "compatible_original_dev": str(compatible_original_dev),
    ...
    print("=" * 80)
    print("开始运行外部领域 4 模型实验（兼容版）")
    print(f"Seeds: {args.seeds}")
    print(f"original_train: {compatible_original_train}")
    print(f"original_dev(test): {compatible_original_dev}")
    print("=" * 80)
    ...
    for cfg in MODEL_CONFIGS:
        ...
        for seed in args.seeds:
            ...
            cmd = build_command(
                ...
                seed=seed,
                compatible_original_train=compatible_original_train,
                compatible_original_dev=compatible_original_dev,
            )
    ...
    final_obj = {
        ...
        "data_prepare_meta": prepare_meta,
    }
